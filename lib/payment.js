...
const mapObject = (obj, keyMapper = mapKey) => {
  const ret = {}
  for (const key in obj) {
    ret[keyMapper(key)] = obj[key]
  }
...
exports.Payment = class Payment {
  ...
  _defaults (params, defaults) {
    ...
    const values = typeof defaults === 'function'
  ...
  async _query (type, params) {
    ...
    const data = this._validate(result)
    return mapObject(data, reverseMapKey)
  }
