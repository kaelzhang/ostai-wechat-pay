...
const sign = exports.sign = (
  ...
  const stringified = `${stringify(nosigned)}&key=${secret}`

  return cipher[sign_type](stringified).toUpperCase()
}
...
class Payment {
  ...
  async requestPayment (params) {
    ...
    payment.paySign = this.sign(payment)
  ...
  async _query (type, params) {
    ...
    const data = await this.validate(result)
  ...
  async validate (xml) {
    ...
    if (data.return_code == RETURN_CODES.FAIL) {
      throw error('WECHAT_RETURN_FAIL', data.return_msg, data)
    }

    if (data.result_code == RETURN_CODES.FAIL) {
      throw error(data.err_code, data.err_code_des, data)
    }

    if (data.appid && this.app_id !== data.appid) {
      throw error('INVALID_APP_ID', 'invalid app id', data)
    }
    ...
    if (
      ...
      throw error('INVALID_MCH_ID', 'invalid mch id', data)
    }

    if (this.sub_mch_id && this.sub_mch_id !== data.sub_mch_id) {
      throw error('INVALID_SUB_MCH_ID', 'invalid sub mch id', data)
    }

    if (data.sign && this.sign(data) !== data.sign) {
      throw error('INVALID_SIGN', 'invalid signature', data)
    }
